...
void exec_SLTI(CPU* cpu, uint32_t instruction)
{
	...
	cpu->regs[rd(instruction)] = (int64_t)cpu->regs[rs1(instruction)] < imm ? (int64_t)0x1 : (int64_t)0x0;
...
void exec_SLTIU(CPU* cpu, uint32_t instruction)
{
	...
	cpu->regs[rd(instruction)] = (uint64_t)cpu->regs[rs1(instruction)] < imm ? (int64_t)0x1 : (int64_t)0x0;
}
...
{
	int64_t imm = imm_I(instruction);
	int64_t msb = cpu->regs[rs1(instruction)] & 0x80000000;
	cpu->regs[rd(instruction)] = (cpu->regs[rs1(instruction)] >> (int64_t)imm) | msb;
}
